...
export default {
  ...
  methods: {
    click (e) {
      e.preventDefault()
      var checkVal = this.checkValue
      if (checkVal.indexOf(this.value) < 0) {
        checkVal.push(this.value)
      } else {
        var i = checkVal.indexOf(this.value)
        checkVal.splice(i, 1)
      }
  ...
  ready () {
    if (this.checked) {
      if (this.checkValue.indexOf(this.value) < 0) {
        this.checkValue.push(this.value)
      }
    }
  }
}
